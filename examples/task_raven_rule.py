...
from dataset import RavenDatasetRule as dataset, RAVENToTensor as ToTensor
from models import *
...
trainset = dataset(args.path, "train", args.img_size, args.config)
validset = dataset(args.path, "val", args.img_size, args.config)
testset = dataset(args.path, "test", args.img_size, args.config)
...
def train(args, epoch):
    ...
    for image, target, rule_target in t:
        from IPython import embed; embed()
        counter += 1
        if args.cuda:
            image = image.cuda()
            target = target.cuda()
            rule_target = rule_target.cuda()

        optimizer.zero_grad()
        output = model(image, target, rule_target)
        loss = compute_loss(args, output, target, rule_target)
        loss.backward()
        optimizer.step()
